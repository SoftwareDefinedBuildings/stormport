...
module SERPRoutingEngineP {
    ...
    uses {
        ...
        interface TrickleTimer as MeshInfoTrickleTimer[uint8_t key];
        interface IPForward;
    }
} implementation {
...
#define compare_ipv6(node1, node2) \
  (!memcmp((node1), (node2), sizeof(struct in6_addr)))
    ...
    /*
     * This method is called when StdControl.start is called.
     * If we are the root, we begin advertising the mesh info on a rapid start
     * TrickleTimer. If we are not root, then we must not know anything about
     * the mesh, so we start bcasting Router Solicitation messages on rapid start
     * Trickle Timer
     */
    task void init() {
        if (I_AM_ROOT) {
  ...
  **/
    ...
    /*
     * This message is broadcast to opportunistically inform our neighbors of our current
     * set of neighbors, current prefix and current neighbor list
     */
    task void send_mesh_info_RA() {
      struct nd_router_advertisement_t ra;
    ...
    /**
     * TODO: if i am *NOT* the root and I overhear an RS, then I schedule an RS in response
     * to that mote (probably make a note of this in neighbor table). I then wait for MIN_TRICKLE_TIMER + (hopcount * MIN_TRICKLE_TIMER) + jitter //rand(0, MIN_TRICKLE_TIMER)
     * for a Mesh Info response. I wait for this time, because the hearing of an RS by the root node should trigger a response
     * w/n MIN_TRICKLE_TIMER because the timer is reset at the root. If I do not hear a MeshInfo, then i bcast one in response to the
     * RS. If i *do* hear a MeshInfo, and the node is not in the neighbor list, then I am still waiting. If i hear a MI and node *is*
     * in the neighbor list, then i can bcast a MI if I can offer a better hop count.
     */
    ...
    event void IP_RS.recv(struct ip6_hdr *hdr,
        ...
        if (err != SUCCESS) {
            printf(ERRORC "Error adding to serp neighbor table from RS\n" RESET);
        }

        // if we hear an RS, then we want to respond to it relatively quickly,
        // so we reset the trickle interval for mesh info
        if (part_of_mesh) {
            printf(ERRORC "RESETTING MESH INFO recv RS and responding\n" RESET);
            call MeshInfoTrickleTimer.reset[meshinfo_key]();
        }
    }

    event void RouterAdvMeshAnnTimer.fired() {
        printf(ERRORC "This shouldn't happen\n" RESET);
    ...
    event void MeshInfoTrickleTimer.fired[uint8_t key]() {
        if (key == meshinfo_key && // check the right timer fired
            part_of_mesh)        // only send Mesh Info if we are part of the mesh
        {
            post send_mesh_info_RA();
            return;
        }

        if (key == rs_key && !I_AM_ROOT) {
            post send_rs_task();
            return;
        }
    }
